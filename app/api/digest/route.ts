...
export async function GET(): Promise<NextResponse<DigestResponse>> {
  try {
    ...
    let formatted = `🗞️ **Daily AI News — ${today}**\n\n`;

    if (dontMiss.length > 0) {
      formatted += `🔥 **DON'T MISS:**\n`;
    ...
    if (news.length > 0) {
      formatted += `📰 **Today's News:**\n`;
      news.forEach(item => {
        const hot = item.isHot ? '🔥 ' : '';
        formatted += `• ${hot}${item.title} (${item.source})\n  → ${item.url}\n`;
      });
      formatted += '\n';
    }

    if (toolOfTheDay) {
      formatted += `💡 **Tool/Resource of the Day:**\n`;
    ...
    return NextResponse.json({
      ...
      news: [],
      toolOfTheDay: null,
      formatted: '❌ Error generating digest. Please try again later.',
      itemCount: 0,
    }, { status: 500 });
  }
}
